...
4.  将路径分配到前面提到的输出文件夹。 在前面的代码块中显示了输出路径的示例。 在这种情况下，`sample1`是 S3 存储桶名称，`DEMO`是其中包含`sample1`的文件夹，其中有六个文件夹-两个数据文件夹，其中包括`.jpg`图像，两个注释文件夹由`.json`文件，输出和检查点文件组成。 请注意，路径必须正确； 否则，可能会产生错误：
