...
如果您的应用程序可以管理其中一台虚拟机发生故障的情况，那么将可抢占实例与 Kubernetes 群集配合使用非常有意义，因为这样可以节省大量成本。
...
*   Cloud Bigtable 最适合读写操作非常繁重的用例； 话虽如此，Bigtable 可以用于流和批处理操作。
...
*   对于 1 GB 的存储空间，向用户收取$ 0.18，写入 100,000 个实体$ 0.18，读取 100,000 个实体$ 0.06 以及删除相同金额的$ 0.02。

# 云数据存储和 AI 应用程序

Cloud Datastore 可以充当大型 Web 应用程序的 AI 和 ML 用例的存储。 GCP 上托管的任何电子商务网站都可以使用数据存储来保存数据，并且使用此数据，可以训练 ML 模型并可以向用户提供所需的建议，进而可以提高客户满意度。
...
文档通过它们在数据库中的位置唯一标识。 默认情况下，Firestore 中存储的所有数据都是加密的，并且可以使用访问控制为用户提供适当的访问权限。 对于 1 GB 的存储空间，向用户收取$ 0.18，写入 100,000 个文档$ 0.18，读取 100,000 个文档$ 0.06 和删除相同数量的文档$ 0.02。
...
*   根据实例类型的不同，每小时的价格范围为$ 0.0150– $ 8.0480。 永久存储的价格为：SSD 为 0.17 美元，HDD 为 0.09 美元，备份为 0.08 美元。
...
*   区域实例的每个节点价格为$ 0.90，而多区域节点的价格为每小时每个节点$ 3。
...
*   Redis 是一个通用的数据库，可以用于很多用例。 通常，人们将 Redis 用作缓存服务，但是它具有各种不同的数据结构，可以为**物联网**（ **IoT** ），ML，流式传输等其他用例提供服务 上。
...
BigQuery 拥有一个友好的用户界面，用户可以从中执行所有操作，还具有一个命令行工具`bqclient` *和*，可用于连接到 BigQuery。 BigQuery 有两种定价模式：“按需付费”，即每 TB 查询处理向用户收取 5 美元的费用；以及“固定费用定价”，即每月约 40,000 美元，为此用户可获得 2,000 个专用插槽用于处理。 存储费用为每月每 GB 0.02 美元； 对于短期存储和长期存储，每月每 GB 0.01 USD。
...
Cloud Dataflow 是用于运行批处理和流应用程序的完全托管服务，并且具有用于运行 AI 和 ML 作业的丰富集成。 它是 Google 提供的无服务器服务，基于 Apache Beam 构建，因此，批处理代码和流代码都可以相互使用。 可以以非常简化的方式用 Java 和 Python 构建 Cloud Dataflow 应用程序。
...
除了已处理数据的定价外，批处理和流作业的 Cloud Dataflow 定价基本相同。 它根据 VCPU，RAM，持久性存储和处理的数据量进行收费。
...
1.  我们已将潜在客户的培训和测试数据集加载到 Cloud Storage 存储桶中。
...
3.  在 GCP 控制台中，单击左上角的导航菜单，然后在存储部分中，单击存储（云存储）。
...
2.  接下来，按照以下步骤从 Cloud Storage 存储桶中的可用数据创建`Leads_Training`和`Leads_Test`表：
    ...
    7.  单击自动检测架构。
    8.  在高级选项**的**中，如果数据集具有标题，则将“标题行”设置为跳过为`1`。
    9.  单击创建表。
...
上面的屏幕快照显示了完整的模型评估详细信息。 如我们所见，SQL 界面训练模型的准确性很高，并且训练数据覆盖率。 得分值还表明该模型是最佳拟合的，而不是评估数据的过拟合或过拟合。 关键是，业务分析师还可以使用 BigQuery 提供的简单 SQL 接口执行模型训练和部署。
...
您可以看到生成的屏幕截图。 请比较模型的预测和基于`prospect_id`的测试数据的`Lead_Stage`列，以查看模型的准确性：
