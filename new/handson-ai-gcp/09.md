...
您可以使用部署在 Google Cloud Storage 存储桶上的模型作为服务端点公开，并且这些服务端点可以由使用 REST API 或`gcloud`工具本身的某些应用使用。 `gcloud`实用程序用于快速测试在线预测。 在实际的生产案例中，您可以使用用 Python 编写的脚本或 cURL 实用程序来使用 Rest API，从而在已部署的模型版本上公开预测功能。
