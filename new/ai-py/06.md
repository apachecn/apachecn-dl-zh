...
分类器的表现以`precision`，`recall`和`f1-scores`为特征。 精度是指分类的准确性，召回率是指检索到的项目数占应检索的项目总数的百分比。 好的分类器将具有较高的精度和较高的查全率，但是通常在这两者之间需要权衡。 因此，我们有`f1-score`来表征。 F1 得分是精度和查全率的谐波平均值，使 F1 得分在精度和查全率之间取得了很好的平衡。
