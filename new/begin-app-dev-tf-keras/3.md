...
| 回归 | 平均绝对百分比误差（**MAPE**） | 预测连续特征。 在使用非标准化范围时具有更好的性能。 | 使用产品属性（例如，价格，类型，目标受众，市场条件）预测产品的销售。 |
...
13.  导航至标题`De-normalizing Predictions`并从`utilities.py`脚本中加载两个函数：
    ...
    每个函数都是使用 NumPy 的矢量方式操作实现的。 它们在相同长度的向量中效果很好。 它们旨在应用于完整的结果集。
...
历元是网络响应于数据通过及其损失函数而调整其权重的次数。 为更多时期运行模型可以使它从数据中学到更多，但同时也存在过拟合的风险。
