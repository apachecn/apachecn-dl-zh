...
前面的脚本将从包含下载图像的文件夹中加载所有图像，使用`python-animeface`库检测人脸，然后**从初始图像中裁剪出人脸**。 然后，裁切后的图像将被调整为`64 x 64`的尺寸。如果要更改图像的尺寸，请相应地更改生成器和判别器的架构。 我们现在准备在我们的网络上工作。
