...
其他更改包括删除`QueueRunner`模块以支持使用`tf.data`，删除图形集合以及更改如何处理变量。 `QueueRunner`模块是一种向模型提供数据以进行训练的方法，但是它比`tf.data`复杂且难于使用，后者现在是将数据提供给模型的默认方法。 在第 3 章和*设计和构造输入数据管道*中，说明了将`tf.data`用于数据管道的其他好处。
