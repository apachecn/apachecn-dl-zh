...
3.  我们可以使用 **Sklearn** 中的`cosine_similarity()`函数在Python中轻松计算。我们可以看到，**猫**和**狗**有相似的向量，因为它们都是动物。
...
5.  在标记化的过程中，还有一个可选的步骤，那就是去除停顿词。歇后语是非常常见的词，对句子的整体意思没有帮助。这些词包括`a`、`I`和`or`等。我们可以使用下面的代码从NLTK中打印出一个完整的列表。
