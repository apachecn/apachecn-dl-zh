...
可以在 [https://www.nltk.org/api/nltk.tokenize.html](https://www.nltk.org/api/nltk.tokenize.html) 上探索使用`nltk`对字符串进行的各种标记化。
...
您可以通过 [https://torchtext.readthedocs.io/en/latest/data.html#field](https://torchtext.readthedocs.io/en/latest/data.html#field) 了解更多有关字段的信息。
...
您可以在 [https://torchtext.readthedocs.io/en/latest/data.html#torchtext-data](https://torchtext.readthedocs.io/en/latest/data.html#torchtext-data) 上了解有关数据集的更多信息。
...
您可以在 [https://torchtext.readthedocs.io/en/latest/data.html?highlight=bucketiter#iterators](https://torchtext.readthedocs.io/en/latest/data.html?highlight=bucketiter#iterators) 上找到有关迭代器参数的更多信息。
...
您可以在 [https://torchtext.readthedocs.io/en/latest/vocab.html?highlight=embedding#module-torchtext.vocab](https://torchtext.readthedocs.io/en/latest/vocab.html?highlight=embedding#module-torchtext.vocab) 上了解有关嵌入的更多信息。
...
您可以在 [https://pytorch.org/docs/stable/nn.html#lstm](https://pytorch.org/docs/stable/nn.html#lstm) 上了解有关 LSTM 的更多信息。
...
您可以在 [https://pytorch.org/docs/stable/nn.html#lstm](https://pytorch.org/docs/stable/nn.html#lstm) 上了解有关多层 LSTM 的更多信息。
...
在`forward()`方法中，我们使用`torch.cat()`连接了向前和向后隐藏状态，并使用了向前和向后 LSTM 的最后一个隐藏状态。 在 PyTorch 中，隐藏状态堆叠为`[forward_layer_0, backward_layer_0, forward_layer_1, backward_layer_1, ..., forward_layer_n, backward_layer_n]`，因此所需的张量为`hidden[-2,:,:], hidden[-1,:,:]`。 串联后，在挤出额外的尺寸后，我们将隐藏的矢量传递到完全连接的层中。
