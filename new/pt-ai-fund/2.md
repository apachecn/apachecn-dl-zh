...
对于其他安装方法，您可以访问 [https://pypi.org/project/torchvision/](https://pypi.org/project/torchvision/) 。 上一章*使用 PyTorch 使用张量*的其余依赖关系保持不变。
...
4.接下来，我们将从`torchvision`中提取数据集并应用转换并创建批次。 为此，我们将首先创建一个训练数据集：
...
您可以在 [https://pytorch.org/docs/stable/nn.html](https://pytorch.org/docs/stable/nn.html) 上查看`nn.Module`和`nn.Sequential`的官方文档。
...
您可以在 [https://pytorch.org/docs/master/nn.html#loss-functions](https://pytorch.org/docs/master/nn.html#loss-functions) 上查看更多损失函数的官方文档。
...
接下来，我们通过将每批 64 幅图像（每幅图像由 28 x 28 像素组成）展平到 784 来重塑图像，从而将张量形状从 64 x 28 x 28 更改为 64 x 784，因为我们的模型期望这种形状 输入。 接下来，我们将此输入发送到模型，并从模型中获得该批次的输出预测，然后将其传递给损失函数，也称为`criterion`； 在那里，它评估了预测班级与实际班级之间的差异。
...
您可以在 [https://pytorch.org/docs/stable/optim.html#torch.optim.Optimizer](https://pytorch.org/docs/stable/optim.html#torch.optim.Optimizer) 上了解有关优化程序的更多信息。
...
您可以在 [https://arxiv.org/abs/1207.0580](https://arxiv.org/abs/1207.0580) 上了解有关辍学的更多信息。
...
在此配方中，我们定义了与以前完全相同的网络，但是用`function.relu`和`function.log_softmax`代替了激活功能和 log softmax，这使我们的代码看起来更加简洁明了。
...
您可以通过 [https://pytorch.org/docs/stable/nn.html#torch-nn-functional](https://pytorch.org/docs/stable/nn.html#torch-nn-functional) 了解更多有关功能 API 的信息。