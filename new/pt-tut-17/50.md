...
> 原文： [https://pytorch.org/tutorials/advanced/dispatcher.html](https://pytorch.org/tutorials/advanced/dispatcher.html)
...
批处理张量允许您按示例方式编写代码，然后在`vmap`调用下运行时自动对其进行批处理。 当前正在开发用于编写批处理规则的 API，但是一旦稳定该 API，就可以通过在 Batched 调度键处注册内核来为操作员添加对`vmap`的支持。
