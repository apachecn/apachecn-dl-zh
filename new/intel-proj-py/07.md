...
在`train`函数中，我们将使用批量生成器根据传递的批量大小生成随机批量。 生成器函数可以定义如下。 请注意，这些函数使用`yield`代替`return`。 通过使用所需参数调用函数，将创建批量的迭代器对象。 可以通过将`next`方法应用于迭代器对象来检索批量。 我们将在每个时期开始时调用生成器函数，以便在每个时期中批量都是随机的。
