...
在本章中，我们定义了嵌入层，但未使用预先训练的嵌入（例如 GloVe，FastText 等）来加载它们。 建议读者使用预训练的词向量嵌入为嵌入层加载，并查看是否会产生更好的结果。 在第 4 章中，*使用 GAN* 进行时装行业中的样式转换，我们将通过与生成性对抗网络（这是现代的革命）进行与时装业中样式转换有关的项目。 人工智能领域。