...
“列表 6.2.1”显示了上图的`tf.keras`代码。 它与“第 1 章”，*用 Keras* 引入高级深度学习中的基于 CNN 的分类器相似，不同之处在于，我们使用`Dense`层来提取`256-dim` 特征。 有两个输出模型，`Encoder[0]`和`Encoder[1]`。 两者都将用于训练 StackedGAN。
