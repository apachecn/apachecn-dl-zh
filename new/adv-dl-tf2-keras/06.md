...
“列表 6.1.4”突出显示了对训练函数所做的更改。 与以前的所有 GAN 相似，区分器和生成器（通过对抗性训练）被交替训练。 在对抗训练期间，区分器的权重被冻结。

通过使用`gan.py plot_images()`函数，样本发生器输出图像每 500 个间隔步被保存一次。
...
从这些验证结果中，我们可以看到，除了生成 MNIST 外观数字的能力之外，InfoGAN 还扩展了条件 GAN（如 CGAN 和 ACGAN）的功能。 网络自动学习了两个可以控制发生器输出的特定属性的任意代码。 有趣的是，如果我们将连续代码的数量增加到 2 以上，可以控制哪些附加属性，可以通过将“列表 6.1.1”的突出显示行中的代码扩展到列表 6.1.4 来实现。
...
1.  从两个噪声代码`z[0]`和`z[1]`的离散标签从 0 变到 9，从正态分布中采样，均值为 0.5，标准偏差为 1.0。 结果显示在“图 6.2.9”中。 我们可以看到 StackedGAN 离散代码可以控制生成器生成的数字：
