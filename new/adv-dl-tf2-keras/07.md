...
图 7.1.3：CycleGAN 模型包含四个网络：生成器`G`，生成器`F`，鉴别器`D`<sub>y</sub> 和鉴别器`D`<sub>x</sub>
...
在 U-Net 结构中，编码器层的输出`e`<sub>ni</sub> 与解码器层的输出`d`<sub>i</sub> ，其中`n`= 4 是编码器/解码器的层数，`i`= 1、2 和 3 是共享信息的层号。
