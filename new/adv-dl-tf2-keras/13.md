...
实现*公式 13.7.3* 的损耗函数也在*清单 13.8.2* 中显示。 请注意，损失函数不使用基本真值。 它只是最小化了 MI 的负估计（从而使 MI 最大化）。 对于此简单的 MINE 模型，未实现移动平均损失。 我们使用*清单 13.8.1* 中的相同函数`sample()`来获得联合和边际样本。
