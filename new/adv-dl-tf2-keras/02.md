...
`resnet_v1()`方法是 ResNet 的模型构建器。 它使用工具功能`resnet_layer(),`来帮助构建`Conv2D-BN-ReLU`的堆栈。
...
其次，类似于 ResNet，随着网络的不断深入，特征映射的大小将减小，从而增加内核的接收域大小。 如果 DenseNet 在合并操作中使用连接，则必须协调大小上的差异。
