...
`x[l]`=`H`（`x`<sub>l-</sub> <sub>1</sub> ）（方程式 2.2。 2）

换句话说，通过`H() =Conv2D-Batch Normalization(BN)-ReLU`将 *1-2* 层上的特征图转换为`x`<sub>1-</sub> <sub>1</sub> 。 应用相同的操作集将`x`<sub>1-</sub> <sub>1</sub> 转换为`x[l]`。 换句话说，如果我们有一个 18 层的 VGG，则在将输入图像转换为第 18 个<sup>的</sup>层特征图之前，有 18 个`H()`操作。

一般而言，我们可以观察到`l`层输出特征图仅直接受先前的特征图影响。 同时，对于 ResNet：

`x`<sub>l-</sub> <sub>1</sub> =`H`（`x`<sub>l-</sub> <sub>2</sub> ）（公式 2.2.3）

`x[l]`= ReLU （F（[HTG6 x <sub>l-</sub> <sub>1</sub> ）+ [`x`<sub>l-</sub> <sub>2</sub> ）（公式 2.2.4）
...
`x[l]`=`H`（`x[0]`，`x`<sub>1 [</sub> ，`x[2]`，`x`<sub>l-</sub> <sub>1</sub> ）（公式 2.4.1）
