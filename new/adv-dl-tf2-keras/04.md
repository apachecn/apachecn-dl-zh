...
在对`strides = 2`进行两个`Conv2DTranspose`之后，特征图的大小将为 28 x 28 x *数量的过滤器*。 每个`Conv2DTranspose`之前都有批量规范化和 ReLU。 最后一层具有*乙状结肠*激活，可生成 28 x 28 x 1 假 MNIST 图像。 将每个像素标准化为与[0，255]灰度级相对应的[0.0，1.0]。 *下面的清单 4.2.1* 显示了`tf.keras`中生成器网络的实现。 定义了一个函数来生成生成器模型。 由于整个代码的长度，我们将列表限制为正在讨论的特定行。
