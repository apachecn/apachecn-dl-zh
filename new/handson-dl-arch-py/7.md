...
在最基本的 GAN 模型中，生成器和判别器都是全连接神经网络。 香草 GAN 的体系结构可以描述如下：
...
**有条件的 GAN**（**CGAN**）通过将标签信息馈送到生成器和判别器，从而希望生成特定标签的数据，从而使我们可以控制要生成的内容。 下图显示了 CGAN 的体系结构：
...
CGAN 认为条件变量是已知的。 因此，在训练期间将条件变量显式馈送到判别器。 相反，InfoGAN 假设条件变量是未知的且是潜在的，我们需要根据训练数据来推断条件变量。 InfoGAN 中的鉴别符负责推导后验`P(c | X)`。 下图显示了 InfoGAN 的体系结构：
