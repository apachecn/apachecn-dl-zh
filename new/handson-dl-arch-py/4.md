...
跳过连接的添加解决了深度网络中的饱和和准确性降低的问题，使体系结构可以具有更多的层而不饱和。 该体系结构由 34 层组成，大部分包含 3 x 3 卷积滤波器。 为了减小特征图的宽度和高度，使用了第 2 步卷积。 最后，使用全局平均池，然后使用 1,000 个单位的全连接层。 在下图中，与典型的 **VGG-19** 和没有残留连接的体系结构相比，您可以观察到 ResNet 体系结构：
