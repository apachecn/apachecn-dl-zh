...
如果您已经阅读了前三章中的 iOS 应用程序，那么您可能更喜欢使用手动构建的 TensorFlow iOS 库而不是 TensorFlow 实验窗格，就像使用手动库方法一样，您可以更好地控制可以使用哪些 TensorFlow 操作 添加更好的模型使您的模型满意，这也是我们决定专注于 TensorFlow Mobile 而不是[第 1 章](../Text/01.html)和 *Mobile TensorFlow* 的 TensorFlow Lite 的原因之一。

因此，尽管您可以在阅读本书时尝试使用 TensorFlow 吊舱，以查看吊舱是否已更新以支持模型中使用的所有操作，但从现在开始，我们将始终使用手动构建的 TensorFlow 库（ 请参见 iOS 应用中[第 3 章](../Text/03.html)，*检测对象及其位置*的*在 iOS 中使用对象检测模型的*部分的步骤 1 和 2）。
