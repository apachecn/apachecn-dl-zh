...
例如，如果`discount_rate`为 0.95，则奖励列表`[1,1,1]`中第一个动作的折扣奖励为`1 + 1 * 0.95 + 1 * 0.95 ** 2 = 2.8525`，并且折扣奖励 第二和最后一个元素是 1.95 和 1； 奖励列表`[1,1,1,1,1]`中第一个动作的折扣奖励为`1 + 1 * 0.95 + 1 * 0.95 ** 2 + 1 * 0.95 ** 3 + 1 * 0.95 ** 4 = 4.5244`，其余动作为 3.7099、2.8525、1.95 和 1 。` [1,1,1]`和`[1,1,1,1,1]`的归一化折扣奖励为`[1.2141, 0.0209, -1.2350]`和`[1.3777, 0.7242,  0.0362, -0.6879, -1.4502]`。 每个规范化的折扣清单按降序排列，这意味着动作持续的时间越长（在情节结束之前），其奖励就越大。
