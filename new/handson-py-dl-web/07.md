...
3.  现在定义上一步中创建的三个处理器方法。 上一步中声明的方法取决于它们自己的内部方法。 这些主要是辅助功能，与本章的重点无关，因此我们将把这些留给您实现该功能，您可以通过研究 AWS Lambda 函数的响应主体文档和 Alexa 技能来创建这些功能。 。 可以[在本章的代码存储库中](http://tiny.cc/HOPDLW_CH7_lfpy)的`lambda_function.py`文件的 65 和 102 行之间找到示例实现。
