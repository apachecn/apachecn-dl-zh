...
在本节中，我们将借助 TF.js 开发一个 Web 应用程序。 该应用程序将包括一个标准的，全栈，支持深度学习的 Web 项目的步骤。 我们将从准备数据开始，然后将简短地研究项目架构，然后，我们将逐步构建所需的组件。
...
`train_data()`功能可以异步运行。 它还将训练的每个阶段的损失打印到我们将要从中运行服务器的控制台。 现在，让我们创建一个将调用`train_data()`函数的 API。
