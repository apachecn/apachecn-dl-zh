...
现在，我们将编写一个`async`函数。 这样做的原因是，不会使调用我们函数的客户端 JavaScript 陷入等待结果的状态。 在我们的程序中，需要花费一些时间才能完成的功能是`train_data()`函数。 此函数执行模型的训练：
...
`doTrain`中间件调用`train_data()`函数并等待其结果。 `train_data()`函数返回`Promise`，以便继续执行而不会冻结。 `next()`函数在`train_data()`函数完成后立即运行，并且仅将程序的执行传递给中间件旁边*链接*的功能，如下所示：
