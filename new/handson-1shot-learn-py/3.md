...
*   **测试集**：来自训练集的采样数据点（`x`，*和*）。
*   **嵌入功能**（ <sub>![](img/23c3a7fe-0fcf-4d10-956d-5bd9810c0232.png)</sub> ）：作为元学习器的一部分，*嵌入* *功能*与暹罗网络非常相似。 经过训练可以预测两个输入是否属于同一类。
...
现在让我们简单介绍一下训练。 随着训练输入数据的到来，它会同时通过元学习者和基础学习者。 在元学习器中，它用于连续学习（更新参数），而在基础学习器中，对输入进行预处理之后，它将*元信息*（*梯度*）传递给元 -学习者。 之后，元学习者使用*元信息*（*梯度*）将参数化更新快速返回给基础学习者，以通过使用慢速权重和快速权重的集成来进行优化（如图所示） 在下图中）。 元网络的基本关键思想是通过处理*元信息*以快速的方式学习权重以进行快速概括。
