...
该模型经过 10 个周期的训练，这意味着每个样本至少要在神经网络上抛出 10 次。 注意在此功能中使用了`train_generator`和`val_generator`。 即使有 12GB+ 的 RAM 和 TPU 加速可用，训练也需要花费相当长的时间（这在任何个人中端设备上都是过大的）。 您将能够观察到运行上述代码的单元下方的训练日志。
