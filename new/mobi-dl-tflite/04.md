...
在本章中，我们介绍了如何使用流行的基于深度学习的 API 服务来使用图像处理。 我们还讨论了如何通过扩展先前创建的基础模型，将其与定制训练模型一起应用。 虽然我们没有明确提及，但是基础模型的扩展是称为**迁移学习**（**TL**）的过程的一部分，其中将在特定数据集上训练的模型导入到 用在完全不同的场景中，几乎没有或只有很少的微调。
