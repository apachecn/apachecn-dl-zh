...
有关发布依赖关系的详细讨论，请参阅[第 2 章](../Text/02.html)，*移动视觉–使用设备上模型的面部检测*。 确保运行`Flutter`软件包以在项目中包含依赖项。 要了解有关`image_picker`插件的更多信息，请访问[这里](https://github.com/flutter/plugins/tree/master/packages/image_picker)。
