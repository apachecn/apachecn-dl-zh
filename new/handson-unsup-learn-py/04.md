...
下载后，可以使用 Pandas 加载 CSV 文件（称为`water-treatment.data`）（当然，必须更改术语`<DATA_PATH>`才能指向文件的确切位置）。 第一列是与特定植物相关的索引，而所有其他值都是数字，可以转换为`float64`。 缺少的值用`'?'` 字符表示，并且由于我们没有其他信息，因此将每个属性的均值设置为：
