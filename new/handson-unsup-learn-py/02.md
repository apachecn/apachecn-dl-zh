...
**向量量化**（**VQ**）是一种利用无监督学习对样本 *x <sub class="calibre20">i</sub> ∈ℜ<sup class="calibre27">进行有损压缩的方法。 N</sup>* （为简单起见，我们假设多维样本被展开）或整个数据集`X`。 主要思想是找到带有许多条目 *C < < N* 的密码本`Q`，并将每个元素与条目 *q <sub class="calibre20">相关联 i</sub> ∈Q* 。 在单个样本的情况下，每个条目将代表一个或多个特征组（例如，可以是均值），因此，该过程可以描述为一种变换`T`，其一般表示为 ：
