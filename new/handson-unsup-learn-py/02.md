...
分数 *s（•）∈（-1，1）*。 当 *s（•）→-1* 时，意味着 *b（•）< < a（•）*，因此样本 *x [i] ∈K [j]* 比其他分配的样本更接近最近的簇`K`[`c`] 到 *K [j]* 。 此情况表示分配错误。 相反，当 *s（•）→1* 时， *b（•）> > a（•）*，因此样本 *x [i]* 与其邻近的*邻居*（属于同一簇）比分配给最近簇的任何其他点更近。 显然，这是最佳条件，也是微调算法时要采用的参考。 但是，由于该索引不是全局索引，因此引入轮廓图很有帮助，该轮廓图显示每个样本所获得的得分，按聚类分组并以降序排序。
...
在先前的公式中， *n [true / pred] （k）*代表属于簇 *k∈K* 的真实/预测样本数。 在这一点上，我们可以计算 *Y [真]* 和 *Y [pre]* 的熵：

![](img/bb659233-4d18-40b0-a974-eb645ad18ab2.png)

考虑到熵的定义， *H（•）*通过均匀分布最大化，而均匀分布又对应于每个分配的最大不确定性。 出于我们的目的，还必须引入 *Y [真]* 的条件熵（表示已知另一个变量的分布的不确定性），前提是`Y`以及其他方法：
