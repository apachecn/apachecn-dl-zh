...
**向量量化**（**VQ**）是一种利用无监督学习对样本 *x <sub class="calibre20">i</sub> ∈ℜ<sup class="calibre27">进行有损压缩的方法。 N</sup>* （为简单起见，我们假设多维样本被展平）或整个数据集`X`。 主要思想是找到带有许多条目 *C < < N* 的密码本`Q`，并将每个元素与条目 *q <sub class="calibre20">相关联 i</sub> ∈Q* 。 在单个样本的情况下，每个条目将代表一个或多个特征组（例如，可以是均值），因此，该过程可以描述为一种变换`T`，其一般表示为 ：
...
我们想用 24 个使用 2×2 正方形区域计算的向量执行 VQ（由包含 2×2×3 特征的展平向量表示）。 但是，我们将使用 K 均值算法来查找质心，而不是从头开始执行该过程。 第一步是收集所有正方形区域，如下所示：
