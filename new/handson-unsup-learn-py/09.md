...
初始方差（与最大邻域成比例）为*σ [0]* ，并且根据时间常数*τ*呈指数衰减。 根据经验，当 *t >4τ*时，*σ（t）≈0* ，因此 *τ*应该设置为 1/4 调整阶段的训练时期数的<sup class="calibre27">至</sup>：*τ＝ 0.25•t [adj]* 。 一旦定义了邻域，就可以根据它们与每个样本的不相似性来更新所有成员的权重， *x [i]* ：
