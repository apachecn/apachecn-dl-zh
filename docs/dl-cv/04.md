...
PASCAL VOC 挑战赛于 2005 年至 2012 年进行。该挑战赛被认为是物体检测技术的基准。 数据集中有 20 个类别。 该数据集包含用于训练和验证的 11,530 张图像，以及针对兴趣区域的 27,450 条注释。 以下是数据集中存在的二十个类：
...
SSD（单发多盒）是所有方法中最快的。 此方法同时预测对象并找到边界框。 在培训期间，可能会有很多负面因素，因此很难否定地挖掘班级失衡。 CNN 的输出具有各种大小的功能。 这些被传递到`3x3`卷积过滤器以预测边界框。
