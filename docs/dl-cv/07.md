...
Kulkarni 等人在[论文](http://www.tamaraberg.com/papers/generation_cvpr11.pdf)中，提出了一种从图像中查找对象和属性并使用它来生成文本的方法。 **条件随机字段**（ **CRF** ）。 传统上，CRF 用于结构化预测，例如文本生成。 生成文本的流程如下所示：
...
Donahue 等人在[论文](https://arxiv.org/pdf/1411.4389.pdf)中，提出了**长期递归卷积架构**（ **LRCN** ） 用于图像字幕的任务。 此模型的体系结构如下所示：
...
Xu 等人在[论文](https://arxiv.org/pdf/1502.03044.pdf)中，提出了一种使用**注意机制**进行图像字幕的方法。 注意机制对图像的某些区域比其他区域赋予更多权重。 注意还可以实现可视化，向我们展示模型生成下一个单词时所关注的位置。 建议的模型如下所示：
