...
在本章中，我通过复制[第 2 章](../Text/02.html)和*的网络和数据开始使用深度学习来解决回归问题*。 我们将做一些简单的添加来添加 TensorBoard 回调。 让我们从修改我们首先构建的`mlp`开始。
...
由于我们已在[第 2 章](../Text/02.html)和*中使用这两种模型编写了日志数据，因此使用了深度学习解决了回归问题*，因此可以使用 TensorBoard 以图形方式比较这两种模型。 打开 TensorBoard 并转到 SCALARS 选项卡。 您应该会看到类似这样的内容。 您可能需要单击 loss 和 val_loss 来展开图形：

![](img/6ed1b1a7-9d13-4b10-91f3-2c4bfb2db2f3.png)

张量板显示模型的损耗图和 val_loss 图
...
该网络的损耗函数被卡住，并且比我们之前的运行要高得多。 什么地方出了错？
