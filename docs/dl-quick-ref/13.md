...
鉴别器的体系结构更像我们在前几章中已经看到的。 它实际上只是一个典型的图像分类器，如下图所示。 输出是 Sigmoid 的，因为鉴别器将预测输入图像是真实图像集的成员的概率。 鉴别器正在解决二分类问题：
...
在不稳定的 GAN 中，鉴别器可能会使发电机过载，并绝对确定发电机是假的。 损失为零，并且没有可用于发送到生成器的梯度，因此它不再可以改善。 网络游戏结束。 解决此问题的最佳方法是降低鉴别器的学习率。 您也可以尝试减少整个鉴别器体系结构中神经元的数量。 但是，您可能会在训练过程的后期错过这些神经元。 最终，调整网络体系结构和超参数是避免这种情况的最佳方法。
...
注意，在建立模型之前，我们将鉴别器的训练属性设置为`False`。 这意味着对于该模型，在反向传播期间，我们将不会更新鉴别器的权重。 正如我们在“栈式训练”部分中提到的，我们将冻结这些权重，仅将发电机的权重与堆栈一起移动。 鉴别器将单独训练。
