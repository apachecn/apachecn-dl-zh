...
让我们定义从一层到另一层的特征图。 当然，我们可以有多个可以从每个隐藏层中独立学习的特征图。 例如，我们可以从 28 x 28 个输入神经元开始处理 MNIST 图像，然后在下一个隐藏的区域中调用 *k* 个特征图，每个特征图的大小为 23 x 23 神经元（步幅为 5 x 5）。 层。
...
6.  实例化 ConvNet 并使用`batch_size=96`将火车运行 50 个周期：
