...
5.  馈送到`feed_dict`的`X`和`Y`数据可以是单个`X`和`Y`点（随机梯度），整个训练集（香草）或批次。

6.  梯度下降的另一个变化是增加了动量项（我们将在[第 3 章](../Text/03.html)和*神经网络-感知器*中找到更多相关信息）。 为此，我们使用优化器`tf.train.MomentumOptimizer()`。 它同时将`learning_rate`和`momentum`作为`init`参数：
...
12.  在 30 个纪元后，我们的准确度为 86.5％，在 50 个纪元后为 89.36％，在 100 个纪元后，准确度提高到 90.91％。
